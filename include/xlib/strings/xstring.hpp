#ifndef XLIB_STRINGS_XSTRING_HPP_
#define XLIB_STRINGS_XSTRING_HPP_
...
#include "xlib/strings/detail/xstring_refcount.hpp"
...
class xstring {
 public:
  ...
  xstring(const std::string& str) {
    auto* refcount = new detail::string_xstring_refcount(str);
    ptr_ = refcount->str().data();
    length_ = refcount->str().length();
    refcount_ = refcount;
  }
  xstring(std::string&& str) {
    auto* refcount = new detail::string_xstring_refcount(std::move(str));
    ptr_ = refcount->str().data();
    length_ = refcount->str().length();
    refcount_ = refcount;
  }
  ...
  xstring(const char* cstr, std::size_t length, auto_release release = auto_release::no_release) : refcount_(nullptr) {
    ...
    if (release != auto_release::no_release) {
      refcount_ = new detail::cstring_xstring_refcount(cstr, release);
    }
  }
  ...
  inline bool operator==(const xstring& other) const noexcept {
    return length_ == other.length_ && (empty() || (std::memcmp(ptr_, other.ptr_, length_) == 0));
  }

  inline bool operator<(const xstring& other) const noexcept { return compare(other) < 0; }

  inline const char* data() const noexcept { return ptr_; }

  inline std::size_t length() const noexcept { return length_; }

  inline bool empty() const noexcept { return length_ == 0; }

  inline std::string str() const {
    if (std::type_index(typeid(*refcount_)) == std::type_index(typeid(detail::string_xstring_refcount))) {
      return static_cast<detail::string_xstring_refcount*>(refcount_)->str();
    } else {
...
#endif  // XLIB_STRINGS_XSTRING_HPP_
