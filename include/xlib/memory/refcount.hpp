...
class refcount : public noncopyable {
 public:
  ...
  int32_t operator*() { return count_.load(std::memory_order_acquire); }

  int32_t operator++() { return count_.fetch_add(1, std::memory_order_relaxed) + 1; }

  int32_t operator++(int) { return count_.fetch_add(1, std::memory_order_relaxed); }
